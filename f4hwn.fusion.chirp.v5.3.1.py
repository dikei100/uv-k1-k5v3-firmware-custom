...
DRIVER_VERSION = "Quansheng UV-K1 / UV-K5 V3 driver ver: 2026/03/30 (c) F4HWN v5.3.1"
...
#                mem.extra.append(rs)
        ...
        # mode
        # Modulation encoding: modulation field selects demod (FM=0, AM=1,
        # USB=2, DIG=3), bandwidth field selects wide(0)/narrow(1).
        # CHIRP modes: FM/NFM/AM/NAM map directly via modulation*2+bandwidth.
        # USB and DIG ignore the bandwidth bit.
        modulation = int(_mem.modulation)
        bandwidth = int(_mem.bandwidth)

        MODUL_TO_MODE = {0: ("FM", "NFM"), 1: ("AM", "NAM"),
                         2: ("USB", "USB"), 3: ("DIG", "DIG")}
        if modulation in MODUL_TO_MODE:
            mem.mode = MODUL_TO_MODE[modulation][bandwidth]
        else:
...
#            dtmf.append(killed_setting)
    ...
    def set_memory(self, memory):
        ...
        # mode
        MODE_TO_MODUL = {"FM": (0, 0), "NFM": (0, 1),
                         "AM": (1, 0), "NAM": (1, 1),
                         "USB": (2, 0), "DIG": (3, 0)}
        mod, bw = MODE_TO_MODUL.get(memory.mode, (0, 0))
